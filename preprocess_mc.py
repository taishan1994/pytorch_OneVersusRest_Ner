...
def convert_bert_example(ex_idx, example: InputExample, tokenizer: BertTokenizer,
    ...
    # ========================
    label_ids = np.zeros((len(labels), len(raw_text)))
    ...
    for ent in entities:
        ...
        if ent_start == ent_end:
            label_ids[label2id[ent_type]][ent_start] = ent2id['B']
        else:
            label_ids[label2id[ent_type]][ent_start] = ent2id['B']
            label_ids[label2id[ent_type]][ent_end] = ent2id['I']
            for i in range(ent_start + 1, ent_end):
                label_ids[label2id[ent_type]][i] = ent2id['I']


    if len(label_ids[0]) > max_seq_len - 2:
        label_ids = label_ids[:, :max_seq_len - 2]

    # 上、下、左、右填充，前面填充0， 后面填充0
    label_ids = np.pad(label_ids, ((0,0),(1,1)), 'constant', constant_values=(0,0))
    

    # pad
    if len(label_ids[0]) < max_seq_len:
        pad_length = max_seq_len - len(label_ids[0])
        label_ids = np.pad(label_ids, ((0,0),(0, pad_length)), 'constant', constant_values=(0,0))  # CLS SEP PAD label都为O


    assert len(label_ids[0]) == max_seq_len
...
def save_file(filename, data, id2ent):
    ...
    for feature,tmp_callback in zip(features, callback_info):
        ...
        labels = feature.labels[:, 1:len(text)+1]
        # print(labels.shape)
        for i in range(len(labels[0])):
          file.write(text[i] + ' ' + str(labels[:, i].T) + '\n')
        file.write('\n')
    file.close()
